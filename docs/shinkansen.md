...
2. **人口・面積・人口密度取得**
   ...
   * データソース: e-Stat API（SSDS基礎データ 2023年）
   * 取得データ: A1101=総人口、B1101=総面積、C1101=人口密度
   * 統計表ID: 0000020201（社会・人口統計体系 市区町村データ 基礎データ）
...
5. **e-Stat API Call** → 自治体コード変換 → API リクエスト → JSON レスポンス解析
6. **Data Processing** → 人口・面積・人口密度算出
...
| e-Stat API エラー | `WARNING` ログ、`population/area/density = None` |
...
| 項目       | 概要                                         |
| -------- | ------------------------------------------ |
| 面積データAPI | e-Stat API の別データセットから正確な面積データを取得 |
...
3. 人口・面積・人口密度データに `None` が各々 10% 未満。
